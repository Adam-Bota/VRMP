...
import { PopupEvent, SessionRealtime, VideoEvent } from "@/types/session";
...
// High-precision time difference calculation between event timestamp and local time
function calculateHighPrecisionTimeDiff(eventTimestamp: any): number {
  try {
    // For Firebase Timestamp, extract nanoseconds for high precision
    if (eventTimestamp && typeof eventTimestamp === "object") {
      // If it's a Firestore Timestamp with nanoseconds
      if (
        eventTimestamp.seconds !== undefined &&
        eventTimestamp.nanoseconds !== undefined
      ) {
        // Convert to nanoseconds for highest precision
        const eventTimeNano =
          eventTimestamp.seconds * 1_000_000_000 + eventTimestamp.nanoseconds;

        // Current time in nanoseconds (approximated from milliseconds)
        const now = new Date();
        const currentTimeNano = now.getTime() * 1_000_000;

        // Calculate difference in seconds
        const diffSeconds = (currentTimeNano - eventTimeNano) / 1_000_000_000;

        console.log(`High-precision time diff: ${diffSeconds.toFixed(6)}s`);

        // Only use if reasonable (less than 30 seconds and positive)
    ...
    // Fallback to standard time compensation
    return calculateTimeCompensation(eventTimestamp);
  } catch (error) {
    console.error("Error in high-precision time calculation:", error);
    return calculateTimeCompensation(eventTimestamp);
  }
}
...
// Send live “popup” event with user info and emoji
export async function sendPopupEvent(
  sessionId: string,
  userId: string,
  userName: string,
  ...
  try {
    ...
    console.error("Error sending popup event:", error);
    throw error;
  }
}
...
export function YouTubePlayerSync({
  ...
  const [video, setVideo] = useState<SessionRealtime["videoState"] | null>(
    null
  );
  const videoEvents = video?.events || [];
  ...
  const participantTimes = video?.participantTimes || {};
  ...
  useEffect(() => {
    ...
    if (videoId && sessionId && isSessionModerator) {
    ...
    const unsubscribe = subscribeToVideoState(sessionId, (video) => {
      try {
        // Process and normalize events to ensure they have valid timestamps
        const normalizedEvents = video?.events.map(normalizeEvent);
        setVideo({
          events: normalizedEvents || [],
          id: video?.id || "",
          participantTimes: video?.participantTimes || {},
        });
      } catch (error) {
  ...
  // participant time updates to db
  useEffect(() => {
    if (!player || !sessionId || !userId || !video) return;
  ...
  }, [player, isPlaying, sessionId, userId, video]);

  // Redirect to home on video change
  useEffect(() => {
    if (!userId || !sessionId) return;

    if (video?.id === "") {
      toast.info("Moderator terminated the video.");
      router.push(`/session/${sessionId}`);
    }
  }, [video?.id, userId, sessionId, router]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const externalEvents = videoEvents.filter(
        (event) =>
          event.type === "popup" ||
          (event.userId !== userId && event.originator !== userId)
      );
      ...
      switch (latestExternalEvent.type) {
        ...
        case "video_change":
          ...
          break;
        case "popup": {
          const { userName, emoji } = latestExternalEvent || {};
          let icon = null;
          let color = "";
          switch (emoji) {
            case "like":
              ...
              break;
            case "dislike":
              icon = <ThumbsDown className="text-gray-500" size={24} />;
              color = "text-gray-500";
              break;
            case "heart":
              ...
              break;
            case "heart-broken":
              icon = <HeartCrack className="text-rose-500" size={24} />;
              color = "text-rose-500";
              break;
            case "laugh":
              icon = <Laugh className="text-yellow-400" size={24} />;
              color = "text-yellow-400";
              break;
            case "sad":
              icon = <Frown className="text-blue-400" size={24} />;
              color = "text-blue-400";
              break;
            case "angry":
              icon = <Angry className="text-red-600" size={24} />;
              color = "text-red-600";
              break;
            default:
          ...
          toast.custom(
            () => (
              <div className="flex items-center gap-3 bg-white rounded-lg dark:bg-gray-950 dark:text-white py-3 px-5 shadow-lg">
                {icon}
                <div>
                  <div className="font-semibold">{userName || "Someone"}</div>
                  <div className={color + " text-base"}>{emoji}</div>
                </div>
              </div>
            ),
            { duration: 10000 }
  ...
  useEffect(() => {
    ...
    const handleSeekChange = (currentTime: number) => {
      try {
        ...
        // Send the seek event
        seekVideo(sessionId, userId, currentTime, currentTime).catch((error) =>
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        syncTime &&
        typeof syncTime.currentTime === "number" &&
        syncTime.currentTime > 0
      ) {
